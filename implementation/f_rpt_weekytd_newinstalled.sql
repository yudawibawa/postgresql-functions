drop function if exists f_rpt_weekytd_newinstalled;

create or replace function f_rpt_weekytd_newinstalled
(
	...
	query1 = '
		select
		  ...
		  ,(
				...
				where
					...
					and (a.job_date between  ww.start_date and ww.end_date)
		...
		(
			...
			where
				...
				and a.disposition = ''RUNNING''
				and a.is_new_installed = 1	
				and (a.job_date between '''||v_min_date||''' and '''||v_max_date||''')
		) as pat, 
		(
			select 
				d::date-6 as start_date
				, d::date as end_date
				, concat(d::date-6,'' - '',d::date) as week
			from generate_series('''||v_max_date||'''::date,'''||v_start_year||''', ''-7d'') d
			order by start_date
		) ww	
						 ';
	
	query2 =        '
				select concat(d::date-6,'' - '',d::date) as week
				from generate_series('''||v_max_date||'''::date,'''||v_start_year||''', ''-7d'') d
				order by d::date-6
			 ';
...
-- cara menggunakan 
select f_rpt_weekytd_newinstalled('24910','','2021-03-04','2021-03-10','viewx2389jkhds98ekd9s','viewx2389jkhds98ekd9s_ori');

select * from viewx2389jkhds98ekd9s_ori;
select * from viewx2389jkhds98ekd9s;

drop view if exists viewx2389jkhds98ekd9s_ori;
drop view if exists viewx2389jkhds98ekd9s;
